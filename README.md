...
Any contributions are admired, no matter how big or small. The Cake site consists of several different sections and each one is described below.
...
The format of an addin file substanially looks like:
...
Note that the `Prerelease` flag can be omitted for non-prerelease packages and controls whether NuGet will strive to download a prerelease version of the package when generating the site.
